...
// | unanchored (probe passes, but the request's anchor was dropped on load).
...
if (!args.includes("--system-prompt-file")) {
  ...
  if (process.env.FAKE_CLAUDE_PROBE_LOG)
    appendFileSync(process.env.FAKE_CLAUDE_PROBE_LOG, "probe\n");
  if (resumeAt === "ambiguous") fail("unexpected probe failure");
  fail(`No message found with message.uuid of: ${arg("--resume-session-at")}`);
}
if (resumeAt === "unanchored" && args.includes("--resume-session-at")) {
  if (process.env.FAKE_CLAUDE_PROBE_LOG)
    appendFileSync(process.env.FAKE_CLAUDE_PROBE_LOG, "anchored-request\n");
  const error = `No message found with message.uuid of: ${arg("--resume-session-at")}`;
...
if (prompt.startsWith("fake:hang")) {
...
else if (prompt.startsWith("fake:malformed")) {
  process.stdout.write("{invalid\n");
  setInterval(() => {}, 1000);
} else if (prompt.startsWith("fake:error"))
  send({
    type: "result",
    subtype: "success",
    is_error: true,
...
else if (prompt.startsWith("fake:permissions"))
  send({ type: "control_request", request: { subtype: "can_use_tool" } });
else if (prompt.startsWith("fake:unexpected-tool"))
  send({ type: "system", subtype: "init", tools: ["Bash"] });
else {
  let input = "";
  ...
  process.stdin.on("end", () => {
    ...
    if (args.includes("--resume"))
      history = readFileSync(arg("--resume"), "utf8")
        .trim()
        .split("\n")
        .map(JSON.parse);
    ...
    stream({
      ...
      index: 0,
      delta: { type: "text_delta", text: "héllo 😀\u2028world\u2029" },
    });
    // Native CLI emits per-block assistant mirrors BEFORE block_stop.
    send({
      type: "assistant",
      message: { content: [{ type: "text", text: "must not duplicate" }] },
    });
    stream({ type: "content_block_stop", index: 0 });
    stream({
      type: "message_delta",
      delta: {
        stop_reason: prompt.startsWith("fake:length")
          ? "max_tokens"
          : "end_turn",
      },
