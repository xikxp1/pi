...
test("toolChoice none excludes schemas even when request hook adds tools", async () => {
  ...
  try {
    ...
    const out = await runRequest(
      ...
      { toolChoice: "none", onPayload: (p) => ({ ...p, tools: [tool] }) },
      { executable, killGraceMs: 25 },
    );
...
test("redacted reasoning payload round trips without exposing opaque data as text", () => {
  const out = emptyMessage(model),
    decoder = new ResponseDecoder(out, [], () => {});
  ...
  const withText = {
    ...out,
    content: [...out.content, { type: "text", text: "answer" }],
  };
  ...
  assert.deepEqual(history[1].content[0], {
    type: "redacted_thinking",
    data: "opaque",
  });
  // A reasoning-only turn is dropped by the CLI on resume, so it is omitted here too.
  const bare = convertMessages(
    [{ role: "user", content: "hi" }, out, { role: "user", content: "next" }],
    model,
  );
  assert.deepEqual(
    bare.map((m) => m.role),
