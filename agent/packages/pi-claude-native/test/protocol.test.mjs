...
test("stream costs track cumulative usage, including cache and output-only updates", () => {
  const priced = {
    ...model,
    cost: { input: 3, output: 15, cacheRead: 0.3, cacheWrite: 3.75 },
  };
  const output = emptyMessage(priced);
  const d = new ResponseDecoder(output, [], () => {}, priced);
  d.event({
    type: "message_start",
    message: {
      usage: {
  ...
  assert.equal(output.usage.cost.output, 0.000015);
  const delta = {
    type: "message_delta",
  ...
  assert.deepEqual(output.usage.cost, {
    input: 0.003,
    output: 0.0015,
    cacheRead: 0.0006,
    cacheWrite: 0.01125,
    total: 0.003 + 0.0015 + 0.0006 + 0.01125,
  });
  assert.equal(output.usage.totalTokens, 6100);
});

test("zero rates and absent usage remain zero", () => {
  const output = emptyMessage(model);
  const d = new ResponseDecoder(output, [], () => {}, {
    cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 },
  });
  d.usage();
  d.usage({ input_tokens: 100, output_tokens: 50 });
  assert.equal(output.usage.cost.total, 0);
});
...
test("missing results are repaired and orphaned results are retained", () => {
  const out = convertMessages(
    [
      ...
      user("steer"),
      result("unknown", "important orphan data"),
    ],
    model,
  );
...
test("cross-provider thinking signatures are dropped, native signatures retained", () => {
  ...
  const messages = [
    user("a"),
    {
      ...assistant([thinking, { type: "text", text: "x" }]),
      provider: "other",
    },
    user("b"),
    assistant([thinking, { type: "text", text: "y" }]),
    user("c"),
  ];
  const out = convertMessages(messages, model);
  assert.equal(out[1].content.length, 1);
  assert.equal(out[3].content[0].signature, "signed");
  assert.throws(
    () => convertMessages([user([{ type: "audio" }])], model),
    /Unsupported/,
  );
});
...
test("transcript parent chains are request-local and complete", () => {
  ...
  const records = transcript(messages, "session", "/temp", model.id)
    .trim()
    .split("\n")
    .map(JSON.parse);
...
function decoder() {
  const output = emptyMessage(model),
    events = [];
  return {
    d: new ResponseDecoder(output, [tool], (e) => events.push(e)),
    output,
    events,
  };
}
...
test("unknown tools, malformed arguments and incomplete streams fail closed", () => {
  ...
  d.event({
    type: "content_block_start",
    index: 0,
    content_block: { type: "tool_use", name: wireName(tool.name), id: "x" },
  });
  d.event({
    type: "content_block_delta",
    index: 0,
