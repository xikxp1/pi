...
// rate_limit_event carries resetsAt but almost never utilization, so the footer
// percentages come from the same OAuth endpoint the /usage command reads.
const USAGE_STATUS_KEY = "claude-bridge-usage";
...
// resetsAt arrives as a bare number with no documented unit. Values below this
// threshold cannot be milliseconds for any near-future date, so treat them as
// unix seconds. Guessing wrong renders a reset time in 1970 or the year 57000.
const MS_EPOCH_THRESHOLD = 1e12;

function resetsAtMs(resetsAt: number): number {
	return resetsAt < MS_EPOCH_THRESHOLD ? resetsAt * 1000 : resetsAt;
}

function formatResetIn(atMs: number): string {
	const minutes = Math.round((atMs - Date.now()) / 60000);
...
function publishUsageStatus(usage: any): void {
	const parts: string[] = [];
	for (const [key, label] of USAGE_WINDOWS) {
		...
		const reset = Number.isNaN(resetsAt) ? "" : ` ↻${formatResetIn(resetsAt)}`;
		parts.push(`${label} ${Math.round(window.utilization)}%${reset}`);
	}
	if (parts.length === 0) return;
	piUI?.setStatus(USAGE_STATUS_KEY, `claude ${parts.join(" · ")}`);
}

function readOAuthToken(): string | undefined {
	try {
		const creds = readFileSync(join(homedir(), ".claude", ".credentials.json"), "utf8");
...
async function refreshUsageStatus(): Promise<void> {
	// The usage endpoint is aggressively rate limited, so back off on failure too.
	if (Date.now() - lastUsageFetch < USAGE_POLL_INTERVAL_MS) return;
	lastUsageFetch = Date.now();
	try {
		const token = readOAuthToken();
		if (!token) {
			debug("usage: no OAuth token found");
			return;
		}
		...
		if (!res.ok) {
			debug(`usage: endpoint returned ${res.status}`);
			return;
		}
		publishUsageStatus(await res.json());
	} catch (err) {
...
async function consumeQuery(
	...
	for await (const message of sdkQuery) {
		...
		// Claude Code emits this after message_stop, when the pi stream is already
		// closed, so it has to be handled before the currentPiStream guard below.
		if (message.type === "rate_limit_event") {
			const info = (message as any).rate_limit_info as SDKRateLimitInfo;
			debug("consumeQuery: rate_limit_event", JSON.stringify(info).slice(0, 300));
			void refreshUsageStatus();
			if (info?.status === "rejected") {
				const resetsAt = info.resetsAt ? new Date(resetsAtMs(info.resetsAt)).toLocaleTimeString() : "unknown";
